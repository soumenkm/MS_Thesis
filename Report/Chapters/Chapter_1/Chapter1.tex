...
kernel alignment. Six of the seven languages improve over training. The exception
is Telugu, which has the fewest dedicated tokens in the Qwen2.5 tokeniser and
whose internal representations change the most while its accuracy stays flat. We
do not find the middle-layer processing in English that the latent-language
hypothesis predicts \citep{wendler2024llamas}. Instead, the per-language outcome
follows tokeniser coverage rather than language identity: when the vocabulary
cannot represent a language at a fine enough level, the reward signal is too
coarse to give a useful gradient.
