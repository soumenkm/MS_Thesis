...
Set1 covers French, Spanish, Vietnamese, Indonesian, Japanese, and Chinese, a mixed
group across several scripts and families. Set6 covers Bengali, Hindi, Tamil,
Telugu, Marathi, Urdu, Kannada, Malayalam, and Punjabi, the group of Indian
...
The downstream tasks are evaluated on the languages those benchmarks support:
Vietnamese, Hindi, and Urdu for XNLI, and Vietnamese, Hindi, and Chinese for XQuAD.
For each of these evaluation languages, the neurons we intervene on are the ones
identified within the set that the language belongs to, so Vietnamese and Chinese
neurons come from the Set1 run and Hindi and Urdu neurons from the Set6 run.
...
\begin{figure}[ht]
	...
	\caption{Number of language-specific neurons identified per language by LAPE, for
		Llama~3.1 and Mistral Nemo on Set1 and Set6. The per-language counts vary by
		language and are a small fraction of the total. Act Prob 90p instead keeps a fixed
		number per language ($573$ for Llama~3.1, $717$ for Mistral Nemo) and is not
		plotted.}
