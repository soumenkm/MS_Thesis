...
\paragraph{(iii) Three-phase commitment.}
...
$0.04$. Telugu sits well below the rest, at about $0.38$ to $0.40$, and stays there
for the whole run. So on this task GRPO does not lift the languages much, and Telugu
in particular does not move at all.
...
\begin{figure}[t]
	...
	\caption{Per-language evaluation accuracy across GRPO training steps, scored by
		the verifier. Six languages stay in a band between about $0.72$ and $0.94$ with
...
seven studied languages. The pattern is the same across prompt languages. For a
non-English prompt, the early and middle layers put most of their renormalized mass
on English, with the prompt's own language near zero, and the target language appears
only in the last two or three layers, where English is still comparable to it. So the
model reads out as English through the body of the stack and surfaces the target
language only at the very end.
...
\subsection{Three-Phase Commitment and the English Lead}
...
highest in the middle phase, and the target language appears only in the late phase
and even there does not clearly overtake English.
...
positive at every depth, meaning English leads the target language throughout. It is
largest in the middle layers, around $0.55$ to $0.62$, smaller in the early layers,
and smaller again in the late layers, around $0.20$ to $0.32$, but it stays positive
to the end. As with the other measurements, the gap is flat across the $620$ steps,
so GRPO does not change the English lead at any depth.
