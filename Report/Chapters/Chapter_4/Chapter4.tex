...
script. For each token in the vocabulary, we decode it to its string form and
assign it a language by the majority Unicode script of its letter characters,
excluding punctuation, digits, and whitespace from the count. Tokens that are
model-internal, byte-fallback, or dominated by a script outside the seven
languages are set aside. Japanese and Chinese share the Han script, so a token
containing any hiragana or katakana is labelled Japanese and a pure-Han token is
labelled Chinese. Counting the tokens assigned to each language gives the
approximate coverage in Table~\ref{tab:grpo-coverage}. The gap is large: Thai has
on the order of $2{,}500$ tokens, Bengali about $46$, and Telugu about $14$. As
the results will show, this gap lines up with the one language GRPO cannot lift.
